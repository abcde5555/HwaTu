...
namespace Teamproject2
{
    class Program
    {
        ...
        public const int SeedMoney = 100;
        ...
        static void Main(string[] args)
        {
            ...
            int round = 1;
            // 선수 중 파산하는 사람이 있을 때까지 라운드를 진행한다.
            while (true)
            {
                ...
                {
                    Console.WriteLine($" P{confirmed_loser.Name} 가 이겼습니다. P{confirmed_loser.Name}에게 충전 하시겠습니까? (1.예 , 2.아니오)");
        ...
        static void RunRound(List<Player> players)
        {
            ...
            // 각 선수들의 족보를 계산하고 카드 변경 의사를 확인한다.
            for (int i = 0; i < players.Count; i++)
            {
                ...
                Console.WriteLine($"P{i}는 카드를 교체하시겠습니까? 당신은 지금 {p.Score}점입니다. (0. 교체 , 1. 미교체) ");
                string Choices = Console.ReadLine();
                int Choiced = int.Parse(Choices);
                if(Choiced == 0)
                {
                    Console.WriteLine($"어떤 카드를 교체하시겠습니까 (0 : {p[0]}, 1 : {p[1]})");
            ...
            Console.WriteLine("각자 고른 코드로 계산합니다.");
            for (int i = 0; i < players.Count; i++)
            {
                Player p = players[i];
            ...
            }else { winner.Money += totalBetMoney;}
        }
